...
function initUI(){
    ...
    case "puzzle":
        onagme = true;
        playButton = new Sprite("play").size(70,70).moveTo(1440,60);
        playButton.event(["mousedown"], function(){
            touches = pows.length, fluzziesin = 0;
            ...
            graphics = nonrenew;
            initLogic();
            if(planning) changeGraphicstoBP();
            else changeGraphicstoNonBP();
        })
        uiL.add(playButton);
        break;
    case "ninja":
        ...
        uiL.add(playButton).add(accelerateButton).add(resetButton)
        break;
    ...
    uiL.add(menuPowers).add(touchIcon).add(touchNumber)
    .add(fluzzyIcon).add(fluzzyNumber).add(passIcon)
    .add(star1Icon).add(star2Icon).add(star3Icon);
}
...
function initPowers(powers){
    ...
    function initP(i){
        ...
        var sm = getPowerAsset(p).moveTo(0, -250+95*i);
        menuPowers.add(sm);
        ...
        sm.event(["mousedown"], function(e){
            if(mode === "puzzle" && !planning) return;
            increaseTouch();
            ...
            case 'accele': addGraphUpdate(function(dt){
                if(mode === "puzzle" && planning) rotateSprite.size(150,150)
                else rotateSprite.size(100,100);
...
var Step = 10, speed = 1, ongame = true, planning = true;
...
function fluzzyDie(fluzzy){
    if(typeof(fluzzy)=== "undefined"){if(fluzziestodelete.length){
        ...
        $.each(fluzziesids, function(i,v){
            i = parseInt(i);
            var ctut = [].concat(fluzzies);
            nl = nl.concat(ctut.splice(i0+1,i-i0-1));
            i0=i;
        });
        var ctut = [].concat(fluzzies);
        nl = nl.concat(ctut.splice(i0+1, fluzzies.length-1-i0))
        }
        fluzzies = nl; delete nl; delete ctut;
...
fluzziestodelete = []; fluzziesids = {};
    ...
    else{ if(!fluzziesids[fluzzy.id])fluzziestodelete.push(fluzzy); fluzziesids[fluzzy.id]=true;}
}
var pass, star1, star2, star3;
function fluzzyEnters(fluzzy, door){
    // SCORE STUFF
    fluzziesin++;
    if(fluzziesin === pass.coolies)
        static.add(new Label("next level").moveTo(door.getPos().x-32.5, door.getPos().y).stroke("rgb(200,255,200)").scale(1.5))
    checkStars();
    // EFFECTS STUFF
    door.op(door);
    if(pass !== true)
        setTimeout(function(){door.cl(door)}, 1000)
